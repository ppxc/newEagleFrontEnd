<template>
  <div class="flex flex-col gap-2 pb-3">
    ...
    <ElCard class="flex-1 art-table-card" style="margin-top: 0; padding: 5px">
      <template #header>
        <div class="flex-cb">
          <h4 class="m-0">定损支付量-年度每月【统计时间：{{ currentMaxTjTime }}】</h4>
...
<script setup lang="ts">
  ...
  defineOptions({ name: 'DingsunZflYearTable' })

  // ==================== 1. 类型定义 ====================
  interface DingsunZflYearData {
    tjdate: string | null
  ...
  const buildComnameSgsOptions = async (tjDate: string) => {
    try {
      const res = await dataReport.axiosRequestDingsunZflYear({ current: 1, size: 9999, tjDate, comnameSgs: '' })
      if (Array.isArray(res)) {
        const set = new Set<string>()
        res.forEach((item: DingsunZflYearData) => {
  ...
  const {
    ...
    core: {
      apiFn: async (params: UseTableParams): Promise<UseTableResult<DingsunZflYearData>> => {
        const queryParams = {
        ...
        const response = await dataReport.axiosRequestDingsunZflYear(queryParams)
        let tableResultData: DingsunZflYearData[] = []
  ...
  const handleRefresh = async () => {
    try {
      const res = await dataReport.axiosRequestDingsunZflYear({ current: 1, size: 9999, tjDate: tableApiParams.value.tjDate, comnameSgs: '' })
  ...
  // ==================== 7. 导出 ====================
  const exportColumns = (item: DingsunZflYearData, index: number) => ({
    序号: index + 1,
    统计日期: item.tjdate,
  ...
  const handleExportCurrent = () => {
    const data = tableData.value as DingsunZflYearData[]
    if (!data.length) {
    ...
    XLSX.utils.book_append_sheet(wb, ws, '定损支付量-年度每月')
    XLSX.writeFile(wb, `定损支付量-年度每月_${dateSuffix()}.xlsx`)
    ElNotification({ title: '成功', message: '导出成功', type: 'success' })
  }

  const handleExportAll = async () => {
    try {
      const res = await dataReport.axiosRequestDingsunZflYear(tableApiParams.value)
      const data = (Array.isArray(res) ? res : []) as DingsunZflYearData[]
      if (!data.length) {
        ElNotification({ title: '提示', message: '暂无数据可导出', type: 'warning' })
        return
      }
      ...
      const wb = XLSX.utils.book_new()
      XLSX.utils.book_append_sheet(wb, ws, '定损支付量-年度每月')
      XLSX.writeFile(wb, `定损支付量-年度每月_全部_${dateSuffix()}.xlsx`)
