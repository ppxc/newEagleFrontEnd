<template>
  <div class="flex flex-col gap-2 pb-3">
    ...
    <ElCard class="flex-1 art-table-card" style="margin-top: 0; padding: 5px">
      <template #header>
        <div class="flex-cb">
          <h4 class="m-0">理算量-年度每月【统计时间：{{ currentMaxTjTime }}】</h4>
...
<script setup lang="ts">
  ...
  const buildComnameSgsOptions = async (tjDate: string) => {
    try {
      const res = await dataReport.axiosRequestLisuanYear({ current: 1, size: 9999, tjDate, comnameSgs: '' })
      if (Array.isArray(res)) {
        const set = new Set<string>()
        res.forEach((item: LisuanYearData) => {
  ...
  const {
    ...
    core: {
      apiFn: async (params: UseTableParams): Promise<UseTableResult<LisuanYearData>> => {
        const queryParams = {
        ...
        const response = await dataReport.axiosRequestLisuanYear(queryParams)
        let tableResultData: LisuanYearData[] = []
  ...
  const handleRefresh = async () => {
    try {
      const res = await dataReport.axiosRequestLisuanYear({ current: 1, size: 9999, tjDate: tableApiParams.value.tjDate, comnameSgs: '' })
  ...
  const handleExportCurrent = () => {
    const data = tableData.value as LisuanYearData[]
    if (!data.length) {
    ...
    XLSX.utils.book_append_sheet(wb, ws, '理算量-年度每月')
    XLSX.writeFile(wb, `理算量-年度每月_${dateSuffix()}.xlsx`)
    ElNotification({ title: '成功', message: '导出成功', type: 'success' })
  }

  const handleExportAll = async () => {
    try {
      const res = await dataReport.axiosRequestLisuanYear(tableApiParams.value)
      const data = (Array.isArray(res) ? res : []) as LisuanYearData[]
      if (!data.length) {
        ElNotification({ title: '提示', message: '暂无数据可导出', type: 'warning' })
        return
      }
      ...
      const wb = XLSX.utils.book_new()
      XLSX.utils.book_append_sheet(wb, ws, '理算量-年度每月')
      XLSX.writeFile(wb, `理算量-年度每月_全部_${dateSuffix()}.xlsx`)
