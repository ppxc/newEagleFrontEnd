<template>
  <div class="flex flex-col gap-2 pb-3">
    ...
    <ElCard class="flex-1 art-table-card" style="margin-top: 0; padding: 5px">
      <template #header>
        <div class="flex-cb">
          <h4 class="m-0">定损支付量-月度每日【统计时间：{{ currentMaxTjTime }}】</h4>
...
<script setup lang="ts">
  ...
  const exportColumns = (item: DingsunZflMonthData, index: number) => ({
    序号: index + 1,
    统计日期: item.tjdate,
  ...
  const handleExportCurrent = () => {
    const data = tableData.value as DingsunZflMonthData[]
    if (!data.length) {
  ...
  const handleExportAll = async () => {
    try {
      const res = await dataReport.axiosRequestDingsunZflMonth(tableApiParams.value)
      const data = (Array.isArray(res) ? res : []) as DingsunZflMonthData[]
      if (!data.length) {
        ElNotification({ title: '提示', message: '暂无数据可导出', type: 'warning' })
        return
      }
