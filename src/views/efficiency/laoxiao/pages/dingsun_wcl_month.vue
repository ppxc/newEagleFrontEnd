<template>
  <div class="flex flex-col gap-2 pb-3">
    ...
    <ElCard class="flex-1 art-table-card" style="margin-top: 0; padding: 5px">
      <template #header>
        <div class="flex-cb">
          <h4 class="m-0">定损完成量-月度每日【统计时间：{{ currentMaxTjTime }}】</h4>
...
<script setup lang="ts">
  ...
  defineOptions({ name: 'DingsunWclMonthTable' })

  interface DingsunWclMonthData {
    tjdate: string | null
  ...
  const {
    ...
    distinctApi: (params) => getDistinctComnames({ table: 'acd_dingsun_wcl_month', ...params }),
    hasComnameSgs: true,
    columnsFactory: () => [
      {
        prop: 'comnameSgs',
  ...
  const exportColumns = (item: DingsunWclMonthData, index: number) => ({
    序号: index + 1,
    统计日期: item.tjdate,
  ...
  const SHEET_NAME = '定损完成量-月度每日'

  const handleExportCurrent = () => {
    const data = tableData.value as DingsunWclMonthData[]
    if (!data.length) {
  ...
  const handleExportAll = async () => {
    try {
      const res = await dataReport.axiosRequestDingsunWclMonth(tableApiParams.value)
      const data = (Array.isArray(res) ? res : []) as DingsunWclMonthData[]
      if (!data.length) {
        ElNotification({ title: '提示', message: '暂无数据可导出', type: 'warning' })
        return
      }
