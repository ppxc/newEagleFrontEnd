...
<script setup lang="ts">
  ...
  const buildComnameSgsOptions = async (tjDate: string) => {
    try {
      const res = await dataReport.axiosRequestChakanYear({ current: 1, size: 9999, tjDate, comnameSgs: '' })
      if (Array.isArray(res)) {
        const set = new Set<string>()
        res.forEach((item: ChakanYearData) => {
  ...
  const {
    ...
    core: {
      apiFn: async (params: UseTableParams): Promise<UseTableResult<ChakanYearData>> => {
        const queryParams = {
        ...
        const response = await dataReport.axiosRequestChakanYear(queryParams)
        let tableResultData: ChakanYearData[] = []
  ...
  const handleRefresh = async () => {
    try {
      const res = await dataReport.axiosRequestChakanYear({ current: 1, size: 9999, tjDate: tableApiParams.value.tjDate, comnameSgs: '' })
  ...
  const handleExportCurrent = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, '查勘量-年度每月')
    XLSX.writeFile(wb, `查勘量-年度每月_${dateSuffix()}.xlsx`)
  ...
  const handleExportAll = async () => {
    try {
      ...
      const wb = XLSX.utils.book_new()
      XLSX.utils.book_append_sheet(wb, ws, '查勘量-年度每月')
      XLSX.writeFile(wb, `查勘量-年度每月_全部_${dateSuffix()}.xlsx`)
