<template>
  <div class="flex flex-col gap-2 pb-3">
    ...
    <ElCard class="flex-1 art-table-card" style="margin-top: 0; padding: 5px">
      <template #header>
        <div class="flex-cb">
          <h4 class="m-0">人伤跟踪量-月度每日【统计时间：{{ currentMaxTjTime }}】</h4>
...
<script setup lang="ts">
  ...
  defineOptions({ name: 'RsGzlMonthTable' })

  interface RsGzlMonthData {
    tjdate: string | null
    comcode: string | null
    comname: string | null
    fenzu: string | null
  ...
  // 人伤跟踪量-月度每日：仅 tjDate 筛选，无 comnameSgs 下拉
  const {
    searchBarRef,
    searchFormState,
    searchItems,
  ...
  const exportColumns = (item: RsGzlMonthData, index: number) => ({
    序号: index + 1,
    地市: item.comname,
    人员: item.username,
    分组合计: item.fenzu,
  ...
  const SHEET_NAME = '人伤跟踪量-月度每日'

  const handleExportCurrent = () => {
    const data = tableData.value as RsGzlMonthData[]
    if (!data.length) {
  ...
  const handleExportAll = async () => {
    try {
      const res = await dataReport.axiosRequestRsGzlMonth(tableApiParams.value)
      const data = (Array.isArray(res) ? res : []) as RsGzlMonthData[]
      if (!data.length) {
        ElNotification({ title: '提示', message: '暂无数据可导出', type: 'warning' })
        return
      }
