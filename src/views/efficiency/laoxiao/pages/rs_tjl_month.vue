<template>
  <div class="flex flex-col gap-2 pb-3">
    ...
    <ElCard class="flex-1 art-table-card" style="margin-top: 0; padding: 5px">
      <template #header>
        <div class="flex-cb">
          <h4 class="m-0">人伤调解量-月度每日【统计时间：{{ currentMaxTjTime }}】</h4>
...
<script setup lang="ts">
  ...
  defineOptions({ name: 'RsTjlMonthTable' })

  interface RsTjlMonthData {
    tjdate: string | null
    comcode: string | null
    comname: string | null
    fenzu: string | null
  ...
  } = useLaoxiaoTable<RsTjlMonthData>({
    pageApi: dataReport.axiosRequestRsTjlMonthPage,
    listApi: dataReport.axiosRequestRsTjlMonth,
    hasComnameSgs: false,
    columnsFactory: () => [
  ...
  const SHEET_NAME = '人伤调解量-月度每日'

  const handleExportCurrent = () => {
    const data = tableData.value as RsTjlMonthData[]
    if (!data.length) {
  ...
  const handleExportAll = async () => {
    try {
      const res = await dataReport.axiosRequestRsTjlMonth(tableApiParams.value)
      const data = (Array.isArray(res) ? res : []) as RsTjlMonthData[]
      if (!data.length) {
        ElNotification({ title: '提示', message: '暂无数据可导出', type: 'warning' })
        return
      }
