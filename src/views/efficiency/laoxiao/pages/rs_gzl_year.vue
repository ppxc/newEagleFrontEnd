...
<script setup lang="ts">
  ...
  const {
    ...
    core: {
      apiFn: async (params: UseTableParams): Promise<UseTableResult<RsGzlYearData>> => {
        const queryParams = {
          current: params.current,
          size: params.size,
          tjDate: tableApiParams.value.tjDate || ''
        }
        const response = await dataReport.axiosRequestRsGzlYear(queryParams)
        let tableResultData: RsGzlYearData[] = []
      ...
      columnsFactory: () => [
        ...
        // usercode / tjYear / jaflag 在 Excel Row3 无中文标题，不显示
  ...
  const handleRefresh = async () => {
    try {
      const res = await dataReport.axiosRequestRsGzlYear({ current: 1, size: 9999, tjDate: tableApiParams.value.tjDate })
  ...
  const handleExportCurrent = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, '人伤跟踪量-年度每月')
    XLSX.writeFile(wb, `人伤跟踪量-年度每月_${dateSuffix()}.xlsx`)
  ...
  const handleExportAll = async () => {
    try {
      ...
      const wb = XLSX.utils.book_new()
      XLSX.utils.book_append_sheet(wb, ws, '人伤跟踪量-年度每月')
      XLSX.writeFile(wb, `人伤跟踪量-年度每月_全部_${dateSuffix()}.xlsx`)
