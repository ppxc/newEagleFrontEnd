<template>
  <div class="flex flex-col gap-2 pb-3">
    ...
    <ElCard class="flex-1 art-table-card" style="margin-top: 0; padding: 5px">
      <template #header>
        <div class="flex-cb">
          <h4 class="m-0">查勘量-月度每日【统计时间：{{ currentMaxTjTime }}】</h4>
...
<script setup lang="ts">
  ...
  const {
    ...
    distinctApi: (params) => getDistinctComnames({ table: 'acd_chakan_month', ...params }),
    hasComnameSgs: true,
    columnsFactory: () => [
      {
        prop: 'comnameSgs',
  ...
  const handleExportAll = async () => {
    try {
      const res = await dataReport.axiosRequestChakanMonth(tableApiParams.value)
      const data = (Array.isArray(res) ? res : []) as ChakanMonthData[]
      if (!data.length) {
        ElNotification({ title: '提示', message: '暂无数据可导出', type: 'warning' })
        return
      }
