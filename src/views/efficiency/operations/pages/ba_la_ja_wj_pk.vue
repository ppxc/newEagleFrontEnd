...
<script setup lang="ts">
  ...
  const fetchAllForDropdown = async (tjDate: string) => {
    ...
    try {
      const res = await dataReport.axiosRequestBaLaJaWjPk({ current: 1, size: 9999, tjDate, tjDate: tableApiParams.value.tjDate || '', comnameSgs: tableApiParams.value.comnameSgs ?? '' })
