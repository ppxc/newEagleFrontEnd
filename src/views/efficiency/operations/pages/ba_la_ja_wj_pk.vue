...
<script setup lang="ts">
  ...
  interface BaLaJaWjPkData {
    ...
    wjl: number | null
    wjlTb: number | null
    sumestipaid: number | null
    sumestipaidTb: number | null
    sumpaid: number | null
    sumpaidTb: number | null
    sumpaidCs: number | null
    sumpaidCsTb: number | null
    sumpaidRs: number | null
    sumpaidRsTb: number | null
    rspkZb: string | null
    rspkZbTb: number | null
  ...
  const fetchAllForDropdown = async () => {
    ...
    try {
      const res = await dataReport.axiosRequestBaLaJaWjPk({
        current: 1,
        size: 9999,
  ...
  const exportColumns = (item: BaLaJaWjPkData, index: number) => ({
    ...
    报案量同比: item.balTb,
    人伤报案量: item.balRs,
    人伤报案量同比: item.balRsTb,
    人伤案件占比: item.rsbaZb,
    人伤案件占比同比: item.rsbaZbTb,
    立案量: item.lal,
    立案量同比: item.lalTb,
    结案量: item.jal,
    结案量同比: item.jalTb,
    未决存量: item.wjl,
    未决存量同比: item.wjlTb,
    未决估计赔款: item.sumestipaid,
    估计赔款同比: item.sumestipaidTb,
    整体结案金额: item.sumpaid,
    整体结案金额同比: item.sumpaidTb,
    车损结案金额: item.sumpaidCs,
    车损结案金额同比: item.sumpaidCsTb,
    人伤结案金额: item.sumpaidRs,
    人伤结案金额同比: item.sumpaidRsTb,
    人伤赔款占比: item.rspkZb,
    人伤赔款占比同比: item.rspkZbTb
  })
