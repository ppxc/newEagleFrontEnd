...
<script setup lang="ts">
  ...
  interface LingjieRyData {
    ...
    ljl1_3: number | null; lj1_3Pp: string | null
    ljl4: number | null; lj4Pp: string | null
    ljlWeek: number | null; ljWeekPp: string | null
  ...
  const searchItems = computed(() => [
    { key: 'tjDate', label: '统计日期', type: 'date', props: { placeholder: '选择统计日期', valueFormat: 'YYYY-MM-DD' } },
    { key: 'groups', label: '片区', type: 'select', props: { placeholder: '请选择片区', options: groupsOptions.value, clearable: true } },
  ...
  const buildOptions = (data: LingjieRyData[]) => {
    ...
    groupsOptions.value = Array.from(gset).sort().map((v) => ({ label: v, value: v }))
    usernameOptions.value = Array.from(uset).sort().map((v) => ({ label: v, value: v }))
  ...
  const exportColumns = (item: LingjieRyData, index: number) => ({
    序号: index + 1,
    部门: item.comname, 小组: item.groups, 人员: item.username, 工号: item.usercode,
    '1-3月零结量': item.ljl1_3, '1-3月零结量占比': item.lj1_3Pp,
    当月零结量: item.ljl4, 当月零结量占比: item.lj4Pp,
    本周零结量: item.ljlWeek, 本周零结案占比: item.ljWeekPp
  })
