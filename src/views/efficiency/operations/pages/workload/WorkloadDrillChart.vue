      ...
      <template #header>
        <div class="flex-cb">
          ...
          <div class="flex gap-2 items-center">
            ...
            <ElTag v-else-if="chartData.length > 0" type="success">{{ chartData.length }} 条线</ElTag>
...
const viewTitle = computed(() => {
  ...
  if (currentLevel.value === 'group') return `小组视图 - ${selectedComname.value || selectedComcode.value}`
  return `员工视图 - ${selectedGroupsname.value || selectedGroupscode.value}`
})

const emptyDescription = computed(() => {
  if (currentLevel.value === 'department') return '暂无部门工作量数据'
  if (currentLevel.value === 'group') return `【${selectedComname.value || '该部门'}】暂无下辖小组数据，可能该部门为汇总部门或数据未导入`
...
const transformGroupData = (data: any[]): SeriesData[] => {
  ...
  console.log('[transformGroupData] 原始数据:', JSON.stringify(data.map(i => ({ groupsName: i.groupsName, groupsCode: i.groupsCode }))))
  console.log('[transformGroupData] 输出:', JSON.stringify(result.map(r => ({ name: r.name, groupsCode: r.groupsCode }))))
  return result
}
...
const transformEmpData = (data: any[]): SeriesData[] => {
  ...
  const sample = result.slice(0, 2).map(r => ({
    name: r.name,
    data: r.data.slice(0, 3).map(d => ({ period: d.period, zl: d.zl, isAbnormal: d.isAbnormal, abnormalType: d.abnormalType }))
  }))
...
const buildUrl = () => {
  ...
  if (searchFormState.value.granularity) params.set('granularity', searchFormState.value.granularity)
  ...
  return query ? `/efficiency/operations/pages/workload/workload-drill?${query}` : '/efficiency/operations/pages/workload/workload-drill'
}
...
// URL 同步：状态变化时更新地址栏
watch([currentLevel, selectedComcode, selectedGroupscode, () => searchFormState.value.startDate, () => searchFormState.value.endDate, () => searchFormState.value.granularity], () => {
  const url = buildUrl()
...
onMounted(() => {
  ...
  if (route.query.granularity) searchFormState.value.granularity = route.query.granularity as string
