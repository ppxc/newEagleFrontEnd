...
<script setup lang="ts">
  ...
  defineOptions({ name: 'JieanlBmTable' })

  interface JieanlBmData {
    tjDate: string | null
    comname: string | null
    hj: number | null
  ...
  const { data: tableData, loading, error: tableError, pagination, fetchData, refreshData, columns, columnChecks } = useTable({
    core: {
      apiFn: async (params: UseTableParams): Promise<UseTableResult<JieanlBmData>> => {
        ...
        const page = (response ?? {}) as UseTableResult<JieanlBmData>
  ...
  const dayVal = (it: JieanlBmData, n: number) => (it as any)[`day${n < 10 ? '0' + n : n}`]

  const exportColumns = (item: JieanlBmData, index: number) => {
    const row: any = { 序号: index + 1, 部门: item.comname, 汇总: item.hj }
    for (let n = 1; n <= 31; n++) row[`${n}号`] = dayVal(item, n)
    return row
  }

  const dateSuffix = () => new Date().toLocaleDateString().replace(/\//g, '-')
  const SHEET = '每日结案量-部门实时'
