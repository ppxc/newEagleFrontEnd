...
<script setup lang="ts">
  ...
  interface Data {
    ...
    comnameSgs: string; xl: string
    jbf: number; pfcb: number; fy: number; glfy: number
    zhcbl: string; zhcblTb: string; zhpl: string; zhplTb: string
  ...
  let isInitialized = false; const xlOptions = ref<SelectOption[]>([])
  ...
  const searchItems = computed(() => [
    ...
    { key: 'xl', label: '险种', type: 'select', props: { placeholder: '请选择险种', options: xlOptions.value, clearable: true } }
  ])
  ...
  const fetchAllForDropdown = async (tjDate: string) => {
    ...
    try {
      const res = await comprehensiveLossRate.axiosRequestZhpflXz({ current: 1, size: 9999, tjDate, tjDate: tableApiParams.value.tjDate || '', xl: tableApiParams.value.xl ?? '' })
      if (Array.isArray(res) && res.length) {
        const set = new Set<string>()
        res.forEach((item) => { if (item.xl) set.add(item.xl) })
        xlOptions.value = Array.from(set).map((name) => ({ label: name, value: name }))
        ElNotification({ title: '提示', message: `已加载：${xlOptions.value.length} 个市公司`, type: 'success' })
      }
    } catch {
      /* ignore */
    }
  ...
  const buildOptions = (data: Data[]) => {
    if (xlOptions.value.length) return
    const set = new Set<string>()
    data.forEach((item) => { if (item.xl) set.add(item.xl) })
    xlOptions.value = Array.from(set).map((name) => ({ label: name, value: name }))
    ElNotification({ title: '提示', message: `已加载：${xlOptions.value.length} 个险种`, type: 'success' })
  }
  const { data: tableData, loading, error: tableError, pagination, fetchData, refreshData, handleSizeChange, columns, columnChecks } = useEfficiencyTable({
    core: {
      apiFn: async (params: UseTableParams): Promise<UseTableResult<Data>> => {
        const queryParams = { current: params.current, size: params.size, tjDate: tableApiParams.value.tjDate || '', xl: tableApiParams.value.xl ?? '' }
  ...
  const handleRefresh = async () => { try { const res = await comprehensiveLossRate.axiosRequestZhpflXz({ current: 1, size: 9999 }); if (Array.isArray(res) && res.length) { buildOptions(res); currentMaxTjTime.value = res[0].maxTjTime || '' } await fetchData() } catch { await fetchData() } }
  ...
  const exportColumns = (item: Data, i: number) => ({
    序号: i + 1, 市公司: item.comnameSgs, 险种: item.xl,
    净保费: item.jbf, 赔付成本: item.pfcb, 费用: item.fy, 管理费用: item.glfy,
    综合成本率: item.zhcbl, 综合成本率同比: item.zhcblTb, 综合利润率: item.zhpl, 综合利润率同比: item.zhplTb
  })
  ...
  const handleExportCurrent = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, '综合赔付率-险种'); XLSX.writeFile(wb, `综合赔付率-险种_${dateSuffix()}.xlsx`)
  ...
  const handleExportAll = async () => {
    try {
      ...
      if (!data.length) { ElNotification({ title: '提示', message: '暂无数据可导出', type: 'warning' }); return }
      const ws = XLSX.utils.json_to_sheet(data.map(exportColumns)); const wb = XLSX.utils.book_new()
      XLSX.utils.book_append_sheet(wb, ws, '综合赔付率-险种'); XLSX.writeFile(wb, `综合赔付率-险种_全部_${dateSuffix()}.xlsx`)
