...
<script setup lang="ts">
  ...
  const fetchAllForDropdown = async (tjDate: string) => {
    ...
    try {
      const res = await accidentYearLossRate.axiosRequestPflsgnKhq({ current: 1, size: 9999, tjDate, tjDate: tableApiParams.value.tjDate || '', comnameSgs: tableApiParams.value.comnameSgs ?? '' })
  ...
  const { data: tableData, loading, error: tableError, pagination, fetchData, refreshData, handleSizeChange, columns, columnChecks } = useEfficiencyTable({
    core: {
      apiFn: async (params: UseTableParams): Promise<UseTableResult<PflsgnKhqData>> => {
        ...
        const response = await accidentYearLossRate.axiosRequestPflsgnKhqPage(queryParams)
        const page = (response ?? {}) as UseTableResult<PflsgnKhqData>
