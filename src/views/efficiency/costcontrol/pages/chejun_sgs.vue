...
<script setup lang="ts">
  ...
  const fetchAllForDropdown = async () => {
    ...
    try {
      const res = await carAvgLoss.axiosRequestChejunSgs({
        current: 1,
        size: 9999,
  ...
  const handleRefresh = async () => {
    try {
      const res = await carAvgLoss.axiosRequestChejunSgs({ current: 1, size: 9999 })
      if (Array.isArray(res) && res.length) {
  ...
  const exportColumns = (item: Data, i: number) => ({
    ...
    '车均定损-2万内-同比': item.lwnCjTb,
    '车均定损（2万以上）': item.lwysCjBn,
    '车均定损-2万以上-同比': item.lwysCjTb,
    '车均换件（元）': item.hjcjBn,
    '车均换件-同比': item.hjcjTb,
    '车均工时（元）': item.gscjBn,
    '车均工时-同比': item.gscjTb,
  ...
  const handleExportCurrent = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, '车均定损-市公司')
    XLSX.writeFile(wb, `车均定损-市公司_${dateSuffix()}.xlsx`)
  ...
  const handleExportAll = async () => {
    try {
      ...
      const wb = XLSX.utils.book_new()
      XLSX.utils.book_append_sheet(wb, ws, '车均定损-市公司')
      XLSX.writeFile(wb, `车均定损-市公司_全部_${dateSuffix()}.xlsx`)
