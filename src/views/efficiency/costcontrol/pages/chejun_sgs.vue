<template>
  <div class="flex flex-col gap-2 pb-3">
    ...
    <ElCard class="flex-1 art-table-card" style="margin-top: 0;padding: 5px;">
      <template #header>
        <div class="flex-cb">
          <h4 class="m-0">车均定损-市公司【统计时间：{{ currentMaxTjTime }}】</h4>
...
<script setup lang="ts">
  ...
  defineOptions({ name: 'ChejunSgsTable' })

  interface Data {
    id: number | null | undefined; tjdate: string | null
    comcodeSgs: string; comnameSgs: string
  ...
  const fetchAllForDropdown = async (tjDate: string) => {
    ...
    try {
      const res = await carAvgLoss.axiosRequestChejunSgs({ current: 1, size: 9999, tjDate, tjDate: tableApiParams.value.tjDate || '', comnameSgs: tableApiParams.value.comnameSgs ?? '' })
  ...
  const { data: tableData, loading, error: tableError, pagination, fetchData, refreshData, handleSizeChange, columns, columnChecks } = useEfficiencyTable({
    core: {
      apiFn: async (params: UseTableParams): Promise<UseTableResult<Data>> => {
        ...
        const response = await carAvgLoss.axiosRequestChejunSgsPage(queryParams)
      ...
      columnsFactory: () => [
        { prop: 'comnameSgs', label: '市公司（定损地）', minWidth: 150, align: 'center', fixed: 'left', sortable: true },
  ...
  const handleRefresh = async () => { try { const res = await carAvgLoss.axiosRequestChejunSgs({ current: 1, size: 9999 }); if (Array.isArray(res) && res.length) { buildDeptOptions(res); currentMaxTjTime.value = res[0].maxTjTime || '' } await fetchData() } catch { await fetchData() } }
  ...
  const exportColumns = (item: Data, i: number) => ({
    序号: i + 1, '市公司（定损地）': item.comnameSgs,
  ...
  const handleExportCurrent = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, '车均定损-市公司'); XLSX.writeFile(wb, `车均定损-市公司_${dateSuffix()}.xlsx`)
    ElNotification({ title: '成功', message: '导出成功', type: 'success' })
  }
  const handleExportAll = async () => {
    try {
      const res = await carAvgLoss.axiosRequestChejunSgs({ tjDate: tableApiParams.value.tjDate || '', comnameSgs: tableApiParams.value.comnameSgs ?? '' })
      const data = (Array.isArray(res) ? res : []) as Data[]
      if (!data.length) { ElNotification({ title: '提示', message: '暂无数据可导出', type: 'warning' }); return }
      const ws = XLSX.utils.json_to_sheet(data.map(exportColumns)); const wb = XLSX.utils.book_new()
      XLSX.utils.book_append_sheet(wb, ws, '车均定损-市公司'); XLSX.writeFile(wb, `车均定损-市公司_全部_${dateSuffix()}.xlsx`)
