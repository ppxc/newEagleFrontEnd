...
import { useMergeFirstColumn } from '../../api/useMergeFirstColumn'
  ...
  interface Data {
    ...
    comnameSgs: string; comname: string
  ...
  const fetchAllForDropdown = async (tjDate: string) => {
    ...
    try {
      const res = await policyYearLossRate.axiosRequestPflbdnZgs({ current: 1, size: 9999, tjDate, tjDate: tableApiParams.value.tjDate || '', comnameSgs: tableApiParams.value.comnameSgs ?? '' })
  ...
  const handleRefresh = async () => { try { const res = await policyYearLossRate.axiosRequestPflbdnZgs({ current: 1, size: 9999 }); if (Array.isArray(res) && res.length) { buildDeptOptions(res); currentMaxTjTime.value = res[0].maxTjTime || '' } await fetchData() } catch { await fetchData() } }
  ...
  const handleExportCurrent = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, '保单年赔付率-支公司'); XLSX.writeFile(wb, `保单年赔付率-支公司_${dateSuffix()}.xlsx`)
  ...
  const handleExportAll = async () => {
    try {
      ...
      if (!data.length) { ElNotification({ title: '提示', message: '暂无数据可导出', type: 'warning' }); return }
      const ws = XLSX.utils.json_to_sheet(data.map(exportColumns)); const wb = XLSX.utils.book_new()
      XLSX.utils.book_append_sheet(wb, ws, '保单年赔付率-支公司'); XLSX.writeFile(wb, `保单年赔付率-支公司_全部_${dateSuffix()}.xlsx`)
