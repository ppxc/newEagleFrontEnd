...
<script setup lang="ts">
  ...
  const searchItems = computed(() => [
    ...
    },
    {
      ...
      type: 'select',
      span: 5,
      props: { placeholder: '请选择使用性质', options: syxzOptions.value, clearable: true }
    }
  ])
  ...
  const fetchAllForDropdown = async (tjDate: string) => {
    ...
    try {
      const res = await accidentYearLossRate.axiosRequestPflsgnSyxzZgs({
        current: 1,
        size: 9999,
        tjDate,
      ...
      if (Array.isArray(res) && res.length) {
        ...
        ElNotification({
          title: '提示',
          message: `已加载：${comOptions.value.length} 个市公司 / ${zgsOptions.value.length} 个支公司 / ${syxzOptions.value.length} 个使用性质`,
          type: 'success'
        })
      }
    } catch {
