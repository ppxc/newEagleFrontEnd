...
<script setup lang="ts">
  ...
  const fetchAllForDropdown = async (tjDate: string) => {
    ...
    try {
      const res = await accidentYearLossRate.axiosRequestPflsgnZgs({ current: 1, size: 9999, tjDate, tjDate: tableApiParams.value.tjDate || '', comnameSgs: tableApiParams.value.comnameSgs ?? '' })
  ...
  const { data: tableData, loading, error: tableError, pagination, fetchData, refreshData, handleSizeChange, columns, columnChecks } = useEfficiencyTable({
    core: {
      apiFn: async (params: UseTableParams): Promise<UseTableResult<PflsgnZgsData>> => {
        ...
        const response = await accidentYearLossRate.axiosRequestPflsgnZgsPage(queryParams)
        const page = (response ?? {}) as UseTableResult<PflsgnZgsData>
  ...
  const handleRefresh = async () => {
    try {
      const res = await accidentYearLossRate.axiosRequestPflsgnZgs({ current: 1, size: 9999 })
      if (Array.isArray(res) && res.length) {
  ...
  const handleExportAll = async () => {
    try {
      const res = await accidentYearLossRate.axiosRequestPflsgnZgs(tableApiParams.value)
