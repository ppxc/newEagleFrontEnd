...
<script setup lang="ts">
  ...
  const fetchAllForDropdown = async () => {
    ...
    try {
      const res = await policyYearLossRate.axiosRequestPflbdnPinpai({
        current: 1,
        size: 9999,
  ...
  const buildOptions = (data: Data[]) => {
    ...
    ElNotification({
      title: '提示',
      message: `已加载：${comOptions.value.length} 个市公司 / ${brandOptions.value.length} 个品牌`,
      type: 'success'
    })
  }
  const {
  ...
  const handleRefresh = async () => {
    try {
      const res = await policyYearLossRate.axiosRequestPflbdnPinpai({ current: 1, size: 9999 })
      if (Array.isArray(res) && res.length) {
        buildOptions(res)
  ...
  const handleExportCurrent = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, '保单年赔付率-品牌')
    XLSX.writeFile(wb, `保单年赔付率-品牌_${dateSuffix()}.xlsx`)
  ...
  const handleExportAll = async () => {
    try {
      ...
      const wb = XLSX.utils.book_new()
      XLSX.utils.book_append_sheet(wb, ws, '保单年赔付率-品牌')
      XLSX.writeFile(wb, `保单年赔付率-品牌_全部_${dateSuffix()}.xlsx`)
