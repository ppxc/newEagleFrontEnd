<template>
  <div class="flex flex-col gap-2 pb-3">
    ...
    <ElCard class="flex-1 art-table-card" style="margin-top: 0;padding: 5px;">
      <template #header>
        <div class="flex-cb">
          <h4 class="m-0">保单年赔付率-品牌【统计时间：{{ currentMaxTjTime }}】</h4>
...
import { useMergeFirstColumn } from '../../api/useMergeFirstColumn'
  ...
  interface Data {
    ...
    comnameSgs: string; brandname: string
  ...
  const DEFAULT_FORM = { tjDate: '', comnameSgs: '', brandname: '' }

  const searchBarRef = ref<any>(null); const currentMaxTjTime = ref<string>('')
  let isInitialized = false; const comOptions = ref<SelectOption[]>([]); const brandOptions = ref<SelectOption[]>([])
  ...
  const buildOptions = (data: Data[]) => {
    ...
    data.forEach((item) => { if (item.comnameSgs) comSet.add(item.comnameSgs); if (item.brandname) brandSet.add(item.brandname) })
  ...
  const { data: tableData, loading, error: tableError, pagination, fetchData, refreshData, handleSizeChange, columns, columnChecks } = useEfficiencyTable({
    core: {
      apiFn: async (params: UseTableParams): Promise<UseTableResult<Data>> => {
        ...
        const response = await policyYearLossRate.axiosRequestPflbdnPinpaiPage(queryParams)
  ...
  const exportColumns = (item: Data, i: number) => ({
    序号: i + 1, 市公司: item.comnameSgs, 品牌: item.brandname,
  ...
  const handleExportCurrent = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, '保单年赔付率-品牌'); XLSX.writeFile(wb, `保单年赔付率-品牌_${dateSuffix()}.xlsx`)
    ElNotification({ title: '成功', message: '导出成功', type: 'success' })
  }
  const handleExportAll = async () => {
    try {
      const res = await policyYearLossRate.axiosRequestPflbdnPinpai(tableApiParams.value)
      const data = (Array.isArray(res) ? res : []) as Data[]
      if (!data.length) { ElNotification({ title: '提示', message: '暂无数据可导出', type: 'warning' }); return }
      const ws = XLSX.utils.json_to_sheet(data.map(exportColumns)); const wb = XLSX.utils.book_new()
      XLSX.utils.book_append_sheet(wb, ws, '保单年赔付率-品牌'); XLSX.writeFile(wb, `保单年赔付率-品牌_全部_${dateSuffix()}.xlsx`)
