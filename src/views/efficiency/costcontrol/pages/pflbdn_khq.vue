<template>
  <div class="flex flex-col gap-2 pb-3">
    ...
    <ElCard class="flex-1 art-table-card" style="margin-top: 0;padding: 5px;">
      <template #header>
        <div class="flex-cb">
          <h4 class="m-0">保单年赔付率-客户群【统计时间：{{ currentMaxTjTime }}】</h4>
...
<script setup lang="ts">
  ...
  defineOptions({ name: 'PflbdnKhqTable' })

  interface Data {
    id: number | null | undefined; tjDate: string | null
    comnameSgs: string; comname: string; khq: string
  ...
  const DEFAULT_FORM = { tjDate: '', comnameSgs: '', khq: '' }

  const searchBarRef = ref<any>(null); const currentMaxTjTime = ref<string>('')
  let isInitialized = false; const comOptions = ref<SelectOption[]>([]); const khqOptions = ref<SelectOption[]>([])
  ...
  const searchItems = computed(() => [
    ...
    { key: 'khq', label: '客户群', type: 'select', props: { placeholder: '请选择客户群', options: khqOptions.value, clearable: true } }
  ])
  ...
  const fetchAllForDropdown = async (tjDate: string) => {
    ...
    try {
      const res = await policyYearLossRate.axiosRequestPflbdnKhq({ current: 1, size: 9999, tjDate, tjDate: tableApiParams.value.tjDate || '', comnameSgs: tableApiParams.value.comnameSgs ?? '', khq: tableApiParams.value.khq ?? '' })
  ...
  const buildOptions = (data: Data[]) => {
    if (comOptions.value.length && khqOptions.value.length) return
    const comSet = new Set<string>(); const khqSet = new Set<string>()
    data.forEach((item) => { if (item.comnameSgs) comSet.add(item.comnameSgs); if (item.khq) khqSet.add(item.khq) })
    ...
    ElNotification({ title: '提示', message: `已加载：${comOptions.value.length} 个市公司 / ${khqOptions.value.length} 个客户群`, type: 'success' })
  }
  const { data: tableData, loading, error: tableError, pagination, fetchData, refreshData, handleSizeChange, columns, columnChecks } = useEfficiencyTable({
    core: {
      apiFn: async (params: UseTableParams): Promise<UseTableResult<Data>> => {
        const queryParams = { current: params.current, size: params.size, tjDate: tableApiParams.value.tjDate || '', comnameSgs: tableApiParams.value.comnameSgs ?? '', khq: tableApiParams.value.khq ?? '' }
        const response = await policyYearLossRate.axiosRequestPflbdnKhqPage(queryParams)
  ...
  const handleRefresh = async () => { try { const res = await policyYearLossRate.axiosRequestPflbdnKhq({ current: 1, size: 9999 }); if (Array.isArray(res) && res.length) { buildOptions(res); currentMaxTjTime.value = res[0].maxTjTime || '' } await fetchData() } catch { await fetchData() } }
  ...
  const exportColumns = (item: Data, i: number) => ({
    序号: i + 1, 市公司: item.comnameSgs, 客户群: item.khq,
  ...
  const handleExportCurrent = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, '保单年赔付率-客户群'); XLSX.writeFile(wb, `保单年赔付率-客户群_${dateSuffix()}.xlsx`)
    ElNotification({ title: '成功', message: '导出成功', type: 'success' })
  }
  const handleExportAll = async () => {
    try {
      const res = await policyYearLossRate.axiosRequestPflbdnKhq(tableApiParams.value)
      const data = (Array.isArray(res) ? res : []) as Data[]
      if (!data.length) { ElNotification({ title: '提示', message: '暂无数据可导出', type: 'warning' }); return }
      const ws = XLSX.utils.json_to_sheet(data.map(exportColumns)); const wb = XLSX.utils.book_new()
      XLSX.utils.book_append_sheet(wb, ws, '保单年赔付率-客户群'); XLSX.writeFile(wb, `保单年赔付率-客户群_全部_${dateSuffix()}.xlsx`)
