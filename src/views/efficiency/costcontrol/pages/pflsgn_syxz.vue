<template>
  <div class="flex flex-col gap-2 pb-3">
    ...
    <ElCard class="flex-1 art-table-card" style="margin-top: 0; padding: 5px">
      <template #header>
        <div class="flex-cb">
          <h4 class="m-0">事故年赔付率-使用性质【统计时间：{{ currentMaxTjTime }}】</h4>
...
<script setup lang="ts">
  ...
  defineOptions({ name: 'PflsgnSyxzTable' })

  interface PflsgnSyxzData {
  ...
  const useOptions = ref<SelectOption[]>([])
  ...
  const searchItems = computed(() => [
    ...
    },
    {
      ...
      label: '使用性质',
      type: 'select',
      props: { placeholder: '请选择使用性质', options: useOptions.value, clearable: true }
    }
  ])
  ...
  const fetchAllForDropdown = async () => {
    ...
    try {
      const res = await accidentYearLossRate.axiosRequestPflsgnSyxz({
        current: 1,
        size: 9999,
  ...
  const buildOptions = (data: PflsgnSyxzData[]) => {
    ...
    data.forEach((item) => {
      ...
      if (item.usenaturename) useSet.add(item.usenaturename)
    })
    ...
    ElNotification({
      title: '提示',
      message: `已加载：${comOptions.value.length} 个市公司 / ${useOptions.value.length} 个使用性质`,
      type: 'success'
    })
  }
  ...
  const {
    ...
    core: {
      apiFn: async (params: UseTableParams): Promise<UseTableResult<PflsgnSyxzData>> => {
        ...
        const response = await accidentYearLossRate.axiosRequestPflsgnSyxzPage(queryParams)
  ...
  const handleRefresh = async () => {
    try {
      const res = await accidentYearLossRate.axiosRequestPflsgnSyxz({ current: 1, size: 9999 })
      if (Array.isArray(res) && res.length) {
        buildOptions(res)
  ...
  const exportColumns = (item: PflsgnSyxzData, index: number) => ({
    序号: index + 1,
    市公司: item.comnameSgs,
  ...
  const handleExportCurrent = () => {
    const data = tableData.value as PflsgnSyxzData[]
    if (!data.length) {
    ...
    XLSX.utils.book_append_sheet(wb, ws, '事故年赔付率-使用性质')
    XLSX.writeFile(wb, `事故年赔付率-使用性质_${dateSuffix()}.xlsx`)
    ElNotification({ title: '成功', message: '导出成功', type: 'success' })
  }

  const handleExportAll = async () => {
    try {
      const res = await accidentYearLossRate.axiosRequestPflsgnSyxz(tableApiParams.value)
      const data = (Array.isArray(res) ? res : []) as PflsgnSyxzData[]
      if (!data.length) {
        ElNotification({ title: '提示', message: '暂无数据可导出', type: 'warning' })
        return
      }
      ...
      const wb = XLSX.utils.book_new()
      XLSX.utils.book_append_sheet(wb, ws, '事故年赔付率-使用性质')
      XLSX.writeFile(wb, `事故年赔付率-使用性质_全部_${dateSuffix()}.xlsx`)
