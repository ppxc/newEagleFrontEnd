...
<script setup lang="ts">
  ...
  const fetchAllForDropdown = async (tjDate: string) => {
    ...
    try {
      const res = await policyYearLossRate.axiosRequestPflbdnPpZgs({
        current: 1,
        size: 9999,
        tjDate,
  ...
  const handleExportCurrent = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, '保单年赔付率-支公司-品牌')
    XLSX.writeFile(wb, `保单年赔付率-支公司-品牌_${dateSuffix()}.xlsx`)
  ...
  const handleExportAll = async () => {
    try {
      ...
      const wb = XLSX.utils.book_new()
      XLSX.utils.book_append_sheet(wb, ws, '保单年赔付率-支公司-品牌')
      XLSX.writeFile(wb, `保单年赔付率-支公司-品牌_全部_${dateSuffix()}.xlsx`)
