<template>
  <div class="flex flex-col gap-2 pb-3">
    <ArtSearchBar ref="searchBarRef" v-model="searchFormState" :items="searchItems" :rules="rules" :is-expand="true" :show-expand="false" :show-reset-button="true" :show-search-button="true" :disabled-search-button="false" @search="handleSearch" @reset="handleReset" />
    <ElCard class="flex-1 art-table-card" style="margin-top: 0;padding: 5px;">
      <template #header>
        <div class="flex-cb">
          <h4 class="m-0">保单年赔付率-支公司-品牌【统计时间：{{ currentMaxTjTime }}】</h4>
...
<script setup lang="ts">
  ...
  let isInitialized = false; const comOptions = ref<SelectOption[]>([]); const zgOptions = ref<SelectOption[]>([]); const brandOptions = ref<SelectOption[]>([])
  ...
  const buildOptions = (data: Data[]) => {
    ...
    const comSet = new Set<string>(); const zgSet = new Set<string>(); const brandSet = new Set<string>()
    data.forEach((item) => { if (item.comnameSgs) comSet.add(item.comnameSgs); if (item.comname) zgSet.add(item.comname); if (item.brandname) brandSet.add(item.brandname) })
  ...
  const { data: tableData, loading, error: tableError, pagination, fetchData, refreshData, columns, columnChecks } = useTable({
    core: {
      apiFn: async (params: UseTableParams): Promise<UseTableResult<Data>> => {
        const queryParams = { current: params.current, size: params.size, tjDate: tableApiParams.value.tjDate || '', comnameSgs: tableApiParams.value.comnameSgs ?? '', comname: tableApiParams.value.comname ?? '', brandname: tableApiParams.value.brandname ?? '' }
        const response = await policyYearLossRate.axiosRequestPflbdnPpZgsPage(queryParams)
  ...
  const handleRefresh = async () => { try { const res = await policyYearLossRate.axiosRequestPflbdnPpZgs({ current: 1, size: 9999 }); if (Array.isArray(res) && res.length) { buildOptions(res); currentMaxTjTime.value = res[0].maxTjTime || '' } await fetchData() } catch { await fetchData() } }
  ...
  const exportColumns = (item: Data, i: number) => ({
    序号: i + 1, 市公司: item.comnameSgs, 支公司: item.comname, 品牌: item.brandname,
  ...
  const handleExportCurrent = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, '保单年赔付率-支公司-品牌'); XLSX.writeFile(wb, `保单年赔付率-支公司-品牌_${dateSuffix()}.xlsx`)
    ElNotification({ title: '成功', message: '导出成功', type: 'success' })
  }
  const handleExportAll = async () => {
    try {
      const res = await policyYearLossRate.axiosRequestPflbdnPpZgs(tableApiParams.value)
      const data = (Array.isArray(res) ? res : []) as Data[]
      if (!data.length) { ElNotification({ title: '提示', message: '暂无数据可导出', type: 'warning' }); return }
      const ws = XLSX.utils.json_to_sheet(data.map(exportColumns)); const wb = XLSX.utils.book_new()
      XLSX.utils.book_append_sheet(wb, ws, '保单年赔付率-支公司-品牌'); XLSX.writeFile(wb, `保单年赔付率-支公司-品牌_全部_${dateSuffix()}.xlsx`)
