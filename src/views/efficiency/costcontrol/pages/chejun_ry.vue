...
import { useMergeFirstColumn } from '../../api/useMergeFirstColumn'
  ...
  const fetchAllForDropdown = async (tjDate: string) => {
    ...
    try {
      const res = await carAvgLoss.axiosRequestChejunRy({ current: 1, size: 9999, tjDate, tjDate: tableApiParams.value.tjDate || '', comnameSgs: tableApiParams.value.comnameSgs ?? '' })
  ...
  const { data: tableData, loading, error: tableError, pagination, fetchData, refreshData, handleSizeChange, columns, columnChecks } = useEfficiencyTable({
    core: {
      ...
      columnsFactory: () => [
        ...
        { prop: 'comname', label: '处理部门', minWidth: 130, align: 'center', fixed: 'left', sortable: true },
        { prop: 'username', label: '处理人员', minWidth: 100, align: 'center', fixed: 'left', sortable: true },
  ...
  const handleRefresh = async () => { try { const res = await carAvgLoss.axiosRequestChejunRy({ current: 1, size: 9999 }); if (Array.isArray(res) && res.length) { buildDeptOptions(res); currentMaxTjTime.value = res[0].maxTjTime || '' } await fetchData() } catch { await fetchData() } }
  ...
  const exportColumns = (item: Data, i: number) => ({
    序号: i + 1, '市公司（定损地）': item.comnameSgs, 处理部门: item.comname, 处理人员: item.username, 处理工号: item.usercode,
  ...
  const handleExportCurrent = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, '车均定损-人员'); XLSX.writeFile(wb, `车均定损-人员_${dateSuffix()}.xlsx`)
    ElNotification({ title: '成功', message: '导出成功', type: 'success' })
  }
  const handleExportAll = async () => {
    try {
      const res = await carAvgLoss.axiosRequestChejunRy({ tjDate: tableApiParams.value.tjDate || '', comnameSgs: tableApiParams.value.comnameSgs ?? '' })
      const data = (Array.isArray(res) ? res : []) as Data[]
      if (!data.length) { ElNotification({ title: '提示', message: '暂无数据可导出', type: 'warning' }); return }
      const ws = XLSX.utils.json_to_sheet(data.map(exportColumns)); const wb = XLSX.utils.book_new()
      XLSX.utils.book_append_sheet(wb, ws, '车均定损-人员'); XLSX.writeFile(wb, `车均定损-人员_全部_${dateSuffix()}.xlsx`)
