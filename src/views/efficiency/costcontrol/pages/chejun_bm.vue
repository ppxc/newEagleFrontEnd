...
<script setup lang="ts">
  ...
  interface Data {
    ...
    comcode: string; dsqy: string
    ajsBn: number | null; ajsQn: number | null
    bncj: number | null; qncj: number | null
    tb: number | null
    dsjeBn: number | null; dsjeQn: number | null
  ...
  let isInitialized = false; const dsqyOptions = ref<SelectOption[]>([])
  ...
  const searchItems = computed(() => [
    ...
    { key: 'dsqy', label: '定损区域', type: 'input', span: 6, props: { placeholder: '模糊搜索定损区域', clearable: true } }
  ])
  ...
  const fetchAllForDropdown = async (tjDate: string) => {
    ...
    try {
      const res = await carAvgLoss.axiosRequestChejunBm({ current: 1, size: 9999, tjDate, tjDate: tableApiParams.value.tjDate || '', dsqy: tableApiParams.value.dsqy ?? '' })
      if (Array.isArray(res) && res.length) {
        const set = new Set<string>()
        res.forEach((item) => { if (item.dsqy) set.add(item.dsqy) })
        dsqyOptions.value = Array.from(set).map((name) => ({ label: name, value: name }))
        ElNotification({ title: '提示', message: `已加载：${dsqyOptions.value.length} 个定损区域`, type: 'success' })
      }
    } catch {
      /* ignore */
    }
  ...
  const buildOptions = (data: Data[]) => {
    if (dsqyOptions.value.length) return
    const set = new Set<string>()
    data.forEach((item) => { if (item.dsqy) set.add(item.dsqy) })
    dsqyOptions.value = Array.from(set).map((name) => ({ label: name, value: name }))
    ElNotification({ title: '提示', message: `已加载：${dsqyOptions.value.length} 个定损区域`, type: 'success' })
  }
  const { data: tableData, loading, error: tableError, pagination, fetchData, refreshData, handleSizeChange, columns, columnChecks } = useEfficiencyTable({
    core: {
      apiFn: async (params: UseTableParams): Promise<UseTableResult<Data>> => {
        const queryParams = { current: params.current, size: params.size, tjDate: tableApiParams.value.tjDate || '', dsqy: tableApiParams.value.dsqy ?? '' }
      ...
      columnsFactory: () => [
        { prop: 'comcode', label: '机构代码', minWidth: 130, align: 'center', fixed: 'left', sortable: true },
        { prop: 'dsqy', label: '定损区域', minWidth: 200, align: 'center', fixed: 'left', sortable: true },
        ...
        { prop: 'dsjeBn', label: '本年定损金额（万元）', width: 170, align: 'center', sortable: true },
        { prop: 'dsjeQn', label: '去年定损金额（万元）', width: 170, align: 'center', sortable: true }
      ]
    },
  ...
  const handleRefresh = async () => { try { const res = await carAvgLoss.axiosRequestChejunBm({ current: 1, size: 9999 }); if (Array.isArray(res) && res.length) { buildOptions(res); currentMaxTjTime.value = res[0].maxTjTime || '' } await fetchData() } catch { await fetchData() } }
  ...
  const exportColumns = (item: Data, i: number) => ({
    序号: i + 1, 机构代码: item.comcode, 定损区域: item.dsqy,
    本年车辆数: item.ajsBn, 去年车辆数: item.ajsQn,
    本年车均: item.bncj, 去年车均: item.qncj, 车均同比: item.tb,
    '本年定损金额（万元）': item.dsjeBn, '去年定损金额（万元）': item.dsjeQn
  })
  ...
  const handleExportCurrent = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, '车均定损-定损区域'); XLSX.writeFile(wb, `车均定损-定损区域_${dateSuffix()}.xlsx`)
    ElNotification({ title: '成功', message: '导出成功', type: 'success' })
  }
  const handleExportAll = async () => {
    try {
      const res = await carAvgLoss.axiosRequestChejunBm({ tjDate: tableApiParams.value.tjDate || '', dsqy: tableApiParams.value.dsqy ?? '' })
      const data = (Array.isArray(res) ? res : []) as Data[]
      if (!data.length) { ElNotification({ title: '提示', message: '暂无数据可导出', type: 'warning' }); return }
      const ws = XLSX.utils.json_to_sheet(data.map(exportColumns)); const wb = XLSX.utils.book_new()
      XLSX.utils.book_append_sheet(wb, ws, '车均定损-定损区域'); XLSX.writeFile(wb, `车均定损-定损区域_全部_${dateSuffix()}.xlsx`)
