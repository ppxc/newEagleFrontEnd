...
import { useMergeFirstColumn } from '../../api/useMergeFirstColumn'
  ...
  const fetchAllForDropdown = async (tjDate: string) => {
    ...
    try {
      const res = await policyYearLossRate.axiosRequestPflbdnSyxz({ current: 1, size: 9999, tjDate, tjDate: tableApiParams.value.tjDate || '', comnameSgs: tableApiParams.value.comnameSgs ?? '', usenaturename: tableApiParams.value.usenaturename ?? '' })
