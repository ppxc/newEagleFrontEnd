...
<script setup lang="ts">
  ...
  const fetchAllForDropdown = async () => {
    ...
    try {
      const res = await policyYearLossRate.axiosRequestPflbdnSyxz({
        current: 1,
        size: 9999,
  ...
  const handleExportCurrent = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, '保单年赔付率-使用性质')
    XLSX.writeFile(wb, `保单年赔付率-使用性质_${dateSuffix()}.xlsx`)
  ...
  const handleExportAll = async () => {
    try {
      ...
      const wb = XLSX.utils.book_new()
      XLSX.utils.book_append_sheet(wb, ws, '保单年赔付率-使用性质')
      XLSX.writeFile(wb, `保单年赔付率-使用性质_全部_${dateSuffix()}.xlsx`)
