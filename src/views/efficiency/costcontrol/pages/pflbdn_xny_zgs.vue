<template>
  <div class="flex flex-col gap-2 pb-3">
    ...
    <ElCard class="flex-1 art-table-card" style="margin-top: 0;padding: 5px;">
      <template #header>
        <div class="flex-cb">
          <h4 class="m-0">保单年赔付率-支公司-新能源【统计时间：{{ currentMaxTjTime }}】</h4>
...
<script setup lang="ts">
  ...
  defineOptions({ name: 'PflbdnXnyZgsTable' })
  interface Data {
    id: number | null | undefined; tjDate: string | null
    comnameSgs: string; comname: string; xnyflag: string
  ...
  const DEFAULT_FORM = { tjDate: '', comnameSgs: '', comname: '', xnyflag: '' }
  ...
  const xnyOptions = ref<SelectOption[]>([])
  ...
  const searchItems = computed(() => [
    ...
    { key: 'xnyflag', label: '新能源', type: 'select', span: 5, props: { placeholder: '请选择新能源', options: xnyOptions.value, clearable: true } }
  ])

  // ==================== 5. 构建下拉 (全量版) ====================
  // 用独立的全量端点（/list, size: 9999）构建市公司/支公司/新能源下拉，
  // 避免首屏分页只有 20 行时遗漏后续页中的支公司/新能源。返回的集合是全量的，与分页结果无关。
  const fetchAllForDropdown = async (tjDate: string) => {
    if (comOptions.value.length && zgsOptions.value.length && xnyOptions.value.length) return
    try {
      const res = await policyYearLossRate.axiosRequestPflbdnXnyZgs({ current: 1, size: 9999, tjDate, comnameSgs: tableApiParams.value.comnameSgs ?? '', comname: tableApiParams.value.comname ?? '', xnyflag: tableApiParams.value.xnyflag ?? '' })
      if (Array.isArray(res) && res.length) {
        const comSet = new Set<string>()
        const zgsSet = new Set<string>()
        const xnySet = new Set<string>()
        res.forEach((item: Data) => {
          if (item.comnameSgs) comSet.add(item.comnameSgs)
          if (item.comname) zgsSet.add(item.comname)
          if (item.xnyflag) xnySet.add(item.xnyflag)
        })
        ...
        xnyOptions.value = Array.from(xnySet).map((name) => ({ label: name, value: name }))
        ElNotification({ title: '提示', message: `已加载：${comOptions.value.length} 个市公司 / ${zgsOptions.value.length} 个支公司 / ${xnyOptions.value.length} 个新能源`, type: 'success' })
      }
    } catch {
      /* ignore */
    }
  ...
  const { data: tableData, loading, error: tableError, pagination, fetchData, refreshData, handleSizeChange, columns, columnChecks } = useEfficiencyTable({
    core: {
      apiFn: async (params: UseTableParams): Promise<UseTableResult<Data>> => {
        const queryParams = { current: params.current, size: params.size, tjDate: tableApiParams.value.tjDate || '', comnameSgs: tableApiParams.value.comnameSgs ?? '', comname: tableApiParams.value.comname ?? '', xnyflag: tableApiParams.value.xnyflag ?? '' }
        const response = await policyYearLossRate.axiosRequestPflbdnXnyZgsPage(queryParams)
      ...
      columnsFactory: () => [
        ...
        { prop: 'xnyflag', label: '新能源', width: 120, align: 'center', fixed: 'left', sortable: true },
  ...
  const exportColumns = (item: Data, i: number) => ({
    序号: i + 1, 市公司: item.comnameSgs, 支公司: item.comname, 新能源: item.xnyflag,
  ...
  const handleExportCurrent = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, '保单年赔付率-支公司-新能源'); XLSX.writeFile(wb, `保单年赔付率-支公司-新能源_${dateSuffix()}.xlsx`)
    ElNotification({ title: '成功', message: '导出成功', type: 'success' })
  }
  const handleExportAll = async () => {
    try {
      const res = await policyYearLossRate.axiosRequestPflbdnXnyZgs(tableApiParams.value)
      const data = (Array.isArray(res) ? res : []) as Data[]
      if (!data.length) { ElNotification({ title: '提示', message: '暂无数据可导出', type: 'warning' }); return }
      const ws = XLSX.utils.json_to_sheet(data.map(exportColumns)); const wb = XLSX.utils.book_new()
      XLSX.utils.book_append_sheet(wb, ws, '保单年赔付率-支公司-新能源'); XLSX.writeFile(wb, `保单年赔付率-支公司-新能源_全部_${dateSuffix()}.xlsx`)
