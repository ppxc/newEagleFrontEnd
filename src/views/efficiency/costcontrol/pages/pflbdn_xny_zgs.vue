...
<script setup lang="ts">
  ...
  const searchItems = computed(() => [
    ...
    },
    {
      ...
      type: 'select',
      span: 5,
      props: { placeholder: '请选择新能源', options: xnyOptions.value, clearable: true }
    }
  ])
  ...
  const fetchAllForDropdown = async (tjDate: string) => {
    ...
    try {
      const res = await policyYearLossRate.axiosRequestPflbdnXnyZgs({
        current: 1,
        size: 9999,
        tjDate,
        comnameSgs: tableApiParams.value.comnameSgs ?? '',
        comname: tableApiParams.value.comname ?? '',
        xnyflag: tableApiParams.value.xnyflag ?? ''
      })
      if (Array.isArray(res) && res.length) {
        ...
        ElNotification({
          title: '提示',
          message: `已加载：${comOptions.value.length} 个市公司 / ${zgsOptions.value.length} 个支公司 / ${xnyOptions.value.length} 个新能源`,
          type: 'success'
        })
      }
    } catch {
  ...
  const handleExportCurrent = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, '保单年赔付率-支公司-新能源')
    XLSX.writeFile(wb, `保单年赔付率-支公司-新能源_${dateSuffix()}.xlsx`)
  ...
  const handleExportAll = async () => {
    try {
      ...
      const wb = XLSX.utils.book_new()
      XLSX.utils.book_append_sheet(wb, ws, '保单年赔付率-支公司-新能源')
      XLSX.writeFile(wb, `保单年赔付率-支公司-新能源_全部_${dateSuffix()}.xlsx`)
