...
<script setup lang="ts">
  ...
  const fetchAllForDropdown = async () => {
    ...
    try {
      const res = await carAvgLoss.axiosRequestChejunClbm({
        current: 1,
        size: 9999,
  ...
  const handleRefresh = async () => {
    try {
      const res = await carAvgLoss.axiosRequestChejunClbm({ current: 1, size: 9999 })
      if (Array.isArray(res) && res.length) {
  ...
  const handleExportCurrent = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, '车均定损-处理部门')
    XLSX.writeFile(wb, `车均定损-处理部门_${dateSuffix()}.xlsx`)
  ...
  const handleExportAll = async () => {
    try {
      ...
      const wb = XLSX.utils.book_new()
      XLSX.utils.book_append_sheet(wb, ws, '车均定损-处理部门')
      XLSX.writeFile(wb, `车均定损-处理部门_全部_${dateSuffix()}.xlsx`)
