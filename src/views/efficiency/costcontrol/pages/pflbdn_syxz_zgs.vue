...
<script setup lang="ts">
  ...
  const handleRefresh = async () => { try { const res = await policyYearLossRate.axiosRequestPflbdnSyxzZgs({ current: 1, size: 9999 }); if (Array.isArray(res) && res.length) { buildDeptOptions(res); currentMaxTjTime.value = res[0].maxTjTime || '' } await fetchData() } catch { await fetchData() } }
