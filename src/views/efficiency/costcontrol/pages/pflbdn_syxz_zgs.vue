<template>
  <div class="flex flex-col gap-2 pb-3">
    ...
    <ElCard class="flex-1 art-table-card" style="margin-top: 0;padding: 5px;">
      <template #header>
        <div class="flex-cb">
          <h4 class="m-0">保单年赔付率-支公司-使用性质【统计时间：{{ currentMaxTjTime }}】</h4>
...
import { useMergeFirstColumn } from '../../api/useMergeFirstColumn'
  ...
  defineOptions({ name: 'PflbdnSyxzZgsTable' })
  interface Data {
    id: number | null | undefined; tjDate: string | null
    comnameSgs: string; comname: string; usenaturename: string
  ...
  const DEFAULT_FORM = { tjDate: '', comnameSgs: '', comname: '', usenaturename: '' }
  ...
  const syxzOptions = ref<SelectOption[]>([])
  ...
  const searchItems = computed(() => [
    ...
    { key: 'usenaturename', label: '使用性质', type: 'select', span: 5, props: { placeholder: '请选择使用性质', options: syxzOptions.value, clearable: true } }
  ])

  // ==================== 5. 构建下拉 (全量版) ====================
  // 用独立的全量端点（/list, size: 9999）构建市公司/支公司/使用性质下拉，
  // 避免首屏分页只有 20 行时遗漏后续页中的支公司/使用性质。返回的集合是全量的，与分页结果无关。
  const fetchAllForDropdown = async (tjDate: string) => {
    if (comOptions.value.length && zgsOptions.value.length && syxzOptions.value.length) return
    try {
      const res = await policyYearLossRate.axiosRequestPflbdnSyxzZgs({ current: 1, size: 9999, tjDate, comnameSgs: tableApiParams.value.comnameSgs ?? '', comname: tableApiParams.value.comname ?? '', usenaturename: tableApiParams.value.usenaturename ?? '' })
      if (Array.isArray(res) && res.length) {
        const comSet = new Set<string>()
        const zgsSet = new Set<string>()
        const syxzSet = new Set<string>()
        res.forEach((item: Data) => {
          if (item.comnameSgs) comSet.add(item.comnameSgs)
          if (item.comname) zgsSet.add(item.comname)
          if (item.usenaturename) syxzSet.add(item.usenaturename)
        })
        ...
        syxzOptions.value = Array.from(syxzSet).map((name) => ({ label: name, value: name }))
        ElNotification({ title: '提示', message: `已加载：${comOptions.value.length} 个市公司 / ${zgsOptions.value.length} 个支公司 / ${syxzOptions.value.length} 个使用性质`, type: 'success' })
      }
    } catch {
      /* ignore */
    }
  ...
  const { data: tableData, loading, error: tableError, pagination, fetchData, refreshData, handleSizeChange, columns, columnChecks } = useEfficiencyTable({
    core: {
      apiFn: async (params: UseTableParams): Promise<UseTableResult<Data>> => {
        const queryParams = { current: params.current, size: params.size, tjDate: tableApiParams.value.tjDate || '', comnameSgs: tableApiParams.value.comnameSgs ?? '', comname: tableApiParams.value.comname ?? '', usenaturename: tableApiParams.value.usenaturename ?? '' }
        const response = await policyYearLossRate.axiosRequestPflbdnSyxzZgsPage(queryParams)
  ...
  const exportColumns = (item: Data, i: number) => ({
    序号: i + 1, 市公司: item.comnameSgs, 支公司: item.comname, 使用性质: item.usenaturename,
  ...
  const handleExportCurrent = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, '保单年赔付率-支公司-使用性质'); XLSX.writeFile(wb, `保单年赔付率-支公司-使用性质_${dateSuffix()}.xlsx`)
    ElNotification({ title: '成功', message: '导出成功', type: 'success' })
  }
  const handleExportAll = async () => {
    try {
      const res = await policyYearLossRate.axiosRequestPflbdnSyxzZgs(tableApiParams.value)
      const data = (Array.isArray(res) ? res : []) as Data[]
      if (!data.length) { ElNotification({ title: '提示', message: '暂无数据可导出', type: 'warning' }); return }
      const ws = XLSX.utils.json_to_sheet(data.map(exportColumns)); const wb = XLSX.utils.book_new()
      XLSX.utils.book_append_sheet(wb, ws, '保单年赔付率-支公司-使用性质'); XLSX.writeFile(wb, `保单年赔付率-支公司-使用性质_全部_${dateSuffix()}.xlsx`)
